...
const Login = props => {
  ...
  // 手机号提交函数
  const handlerSubmit = useCallback(async (e) => {
    ...
    return (
      <>
        <View style={{padding: 20}}>
          <Text style={{fontSize: 20, color: "#888", fontWeight:"bold"}}>手机号注册登录</Text>
        ...
        <View style={{width:"90%", height:40, alignSelf:"center"}}>
          <LinearGradientBtn style={{color:"#fff"}} color={['#9b63cd', '#e0708c']} onPress={handlerSubmit} style={{borderRadius: 20}}>获取验证码</LinearGradientBtn>
        </View>
      </>
    )
  }
  ...
  const renderCode = props => {
    return (
      <>
        <View style={{padding: 20}}>
          <Text style={{fontSize: 20, color: "#888", fontWeight:"bold"}}>请输入六位验证码</Text>
          <Text style={{marginTop:30}}>已发送至+86 {phone}</Text>
          <View>
            <CodeFieldInput value={value} onPress={setValue} onSubmit={handlerOnsubmit}></CodeFieldInput>
          </View>
        </View>
        <View style={{width:"90%", height:40, alignSelf:"center",marginTop:50 }}>
          <LinearGradientBtn disable={isClick} color={['#9b63cd', '#e0708c']} onPress={handlerSubmit} style={{borderRadius: 20, color:"#fff"}}>{btnText}</LinearGradientBtn>
        </View>
      </>
    )
  }
  ...
  return (
    <>
      <View style={{flex: 1}}>
        ...
        {/*设置标题栏*/}
        <StatusBar backgroundColor="transparent" trunslucent={true}></StatusBar>

        {/*设置背景颜色*/}
        <Image style={{width: "100%", height: 200}} source={require("../../assets/images/profileBackground.jpg")}></Image>
